                    ...
                    <div class="flex flex-col justify-start items-center">
                        <form action="{{route('skus.update',$sku)}}" method="post" >
                            ...
                            @method('PUT')
                                ...
                                <x-input class="bg-indigo-100" type="text" name="title" value="{{$sku->title}}" />
                                @if($errors->has('title'))<div class="text-red-500 text-xs">{{ $errors->first('title') }}</div>@endif

                                <input class="checked:bg-blue-500 checked:border-blue-500 focus:outline-none" type="checkbox" name="status" @if ($sku->status == 1)
                                        checked
                                    @endif >

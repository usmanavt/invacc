                ...
                <div class="flex gap-4">
                    ...
                    <div class="flex flex-col justify-start items-center border rounded-md px-2">
                        <form action="{{ route('skus.store') }}" method="post" >
                            @csrf
                            <p class="font-semibold pb-6">Create New Sku</p>
                            <x-label for="title" value="Title"/>
                            <x-input id="title" type="text" name="title"  required minlength="2" :value="old('title')"/>
...
<script>
    ...
    const getMaster = @json(route('skus.master'));
    let table;
    let searchValue = "";
    //  Table Filter

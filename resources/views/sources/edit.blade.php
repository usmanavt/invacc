                    ...
                    <div class="flex flex-col justify-start items-center">
                        <form action="{{route('sources.update',$source)}}" method="post" >
                            ...
                            @method('PUT')
                                ...
                                <x-input class="bg-indigo-100" type="text" name="title" value="{{$source->title}}" />
                                @if($errors->has('title'))<div class="text-red-500 text-xs">{{ $errors->first('title') }}</div>@endif

                                <input class="checked:bg-blue-500 checked:border-blue-500 focus:outline-none" type="checkbox" name="status" @if ($source->status == 1)
                                        checked
                                    @endif >

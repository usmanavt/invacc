            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="px-12 py-2" >
                    <div class="flex flex-col justify-start items-center">
                        <form action="{{route('cares.update',$care)}}" method="post" >
                            @csrf
                            @method('PUT')
                                <x-label for="title" value="Title" class="w-24"/>
                                <x-input id="title" class="bg-indigo-100" type="text" name="title" value="{{$care->title}}" />
                                @if($errors->has('title'))<div class="text-red-500 text-xs">{{ $errors->first('title') }}</div>@endif

                                <input class="checked:bg-blue-500 checked:border-blue-500 focus:outline-none" type="checkbox" name="status" @if ($care->status == 1)
                                        checked
                                @endif >

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Units
            <a class="text-sm text-green-500 hover:text-gray-900" href="{{route('units.create')}}">
                {{-- Add Icon --}}
                            ...
                            <table class="min-w-full divided-y divide-gray-500">
								...
								<tbody class="bg-white divide-y divide-gray-200">
									@foreach($units as $unit)
										<tr class="border-b">
											<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{$unit->unitname}}</td>
											<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                <a class="text-sm text-indigo-500 hover:text-gray-900" href="{{route('units.edit',$unit->id)}}">

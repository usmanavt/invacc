        ...
        <div class="max-w-7xl mx-auto sm:px-2 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="px-12 py-2" >
                    ...
                    <form action="{{ url('/grouprelation/update')."/".  $grouprelation->id   }} " method="post" >
                        @csrf
                                    ...
                                    <div>
                                        ...
                                        <select required name="itmid0" id="itmid0" class="block  bg-slate-200 text-sm rounded-lg ">
                                            ...
                                            @foreach($grpdta as $grplist)
                                                <option value="{{$grplist->grpid}}" {{ $grplist->grpid==$grouprelation->itmid0? 'selected':'' }} >  {{$grplist->grpname}}  </option>
                                             @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select required name="itmid" id="itmid" class="block  bg-slate-200 text-sm rounded-lg ">
                                            ...
                                            @foreach($itmdta as $itmlist)
                                                <option value="{{$itmlist->itmid}}" {{ $itmlist->itmid==$grouprelation->itmid? 'selected':'' }} >  {{$itmlist->itmname}}  </option>
                                             @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select required name="itmsizeid" id="itmsizeid" class="block  bg-slate-200 text-sm rounded-lg ">
                                            ...
                                            @foreach($szedta as $szelist)
                                                <option value="{{$szelist->szeid}}" {{ $szelist->szeid==$grouprelation->itmsizeid? 'selected':'' }} >  {{$szelist->szename}}  </option>
                                             @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div>
                                            ...
                                            <select required name="srcid" id="srcid" class="block  bg-slate-200 text-sm rounded-lg ">
                                                ...
                                                @foreach($srcdta as $srclist)
                                                    <option value="{{$srclist->sid}}" {{ $srclist->sid==$grouprelation->srcid? 'selected':'' }} >  {{$srclist->sname}}  </option>
                                                 @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select required name="brandid" id="brandid" class="block  bg-slate-200 text-sm rounded-lg ">
                                                ...
                                                @foreach($brddta as $brdlist)
                                                    <option value="{{$brdlist->bid}}" {{ $brdlist->bid==$grouprelation->brandid? 'selected':'' }} >  {{$brdlist->bname}}  </option>
                                                 @endforeach
                                            </select>
                                        </div>
                                        <div>
                                            ...
                                            <select required name="purunitid" id="purunitid" class="block  bg-slate-200 text-sm rounded-lg ">
                                                ...
                                                @foreach($untdta as $untlist)
                                                    <option value="{{$untlist->uid}}" {{ $untlist->uid==$grouprelation->purunitid? 'selected':'' }} >  {{$untlist->uname}}  </option>
                                                 @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select required name="locid" id="locid" class="block  bg-slate-200 text-sm rounded-lg ">
                                                ...
                                                @foreach($locdta as $loclist)
                                                    <option value="{{$loclist->lid}}" {{ $loclist->lid==$grouprelation->locid? 'selected':'' }} >  {{$loclist->lname}}  </option>
                                                 @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="flex flex-row justify-start space-x-12 items-center mt-6">
                                <div>
                                    ...
                                    <x-input id="obqty" class="block  bg-slate-200 w-96" type="obqty" name="obqty" value="{{$grouprelation->obqty}}"  />
                                </div>
                                <div>
                                    <x-label for="purrate" :value="__('Purchase Rate')" />
                                    <x-input id="purrate" class="block  bg-slate-200" type="purrate" name="purrate" value="{{$grouprelation->purrate}}"  />
                                </div>

                                <div>
                                    <x-label for="costrate" :value="__('Cost Rate')" />
                                    <x-input id="costrate" class="block  bg-slate-200" type="costrate" name="costrate" value="{{$grouprelation->costrate}}"  />
                                </div>
                            </div>
                                ...
                                <div class="flex flex-row justify-start space-x-12 items-center mt-8">
                                    <div class="form-check">
                                        <input class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" value="Active" type="radio" name="sstatus" id="flexRadioDefault1" {{ ($grouprelation->sstatus=="Active")? "checked":"" }} >
                                        <label class="form-check-label inline-block text-gray-800" for="flexRadioDefault1">
                                        Active
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" value="Stop" type="radio" name="sstatus" id="flexRadioDefault2" {{ ($grouprelation->sstatus=="Stop")? "checked":"" }} >

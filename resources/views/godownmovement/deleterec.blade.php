<x-app-layout>
    ...
    <x-slot name="header">
        <div style="font-size: 3rem;font-weight: bold;color:brown;border:blue">
            Delete Record Information For Godown Movement
        </div>
    </x-slot>
                ...
                <div class="px-6 py-2">
                    ...
                    <div class="grid grid-cols-1">
                        ...
                        <div class="grid grid-cols-12 gap-2 py-2 items-center">
                            ...
                            <select autocomplete="on" class="col-span-2" name="fromg" id="fromg" >
                                @foreach($locations as $location)
                                @if ($location->id == $godownmovement->fromg)
                            ...
                            <select autocomplete="on" class="col-span-2" name="tog" id="tog" >
                                @foreach($locations as $location)
                                @if ($location->id == $godownmovement->tog)
                                    <option value="{{$location->id}}" selected> {{$location->title}} </option>
                                @endif
                                <option value="{{$location->id}}"> {{$location->title}} </option>
                                @endforeach
                            ...
                            <x-input-text title="STO Seq.#" name="stono" id="stono" value="{{ $godownmovement->stono }}"      required   />
                        </div>
                        <div class="grid grid-cols-12 gap-1 py-2 items-center">
                            <x-input-date title="STO Date" id="stodate" name="stodate"  value="{{ $godownmovement->stodate }}" req required class="col-span-2" />
                            <label for="">
                            ...
                            <x-input-numeric title="cstatus" name="cstatus" id="cstatus" hidden  value="{{ $godownmovement->clrldstatus }}"   />
...
function validateForm()
{
    ...
    //     var per= document.getElementById("per");
            ...
            if(cstatus.value == 1)
                {
                    showSnackbar("Godown Movement Transaction Used in GatePasse ");
                    // fromg.focus();
                    return;
                }
    ...
    var data = { 'godownmovement' : dynamicTableData,'godown_movement_id':godown_movement_id.value, 'fromg': fromg.value,'tog': tog.value,'stodate':stodate.value,'stono':stono.value,'mremarks':mremarks.value};




    fetch(@json(route('godownmovement.del')),{
    ...
    .then( response => {
        if (response == 'success')
        {
            window.open(window.location.origin + "/godownmovement","_self" );
        }
    })
    .catch(error => {

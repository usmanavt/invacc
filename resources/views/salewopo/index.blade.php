<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Delivery Challan Without Purchase Order
            {{-- Create New Customer --}}
            <a class="text-sm text-green-500 hover:text-gray-900" href="{{route('salewopo.create')}}">
                {{-- Add Icon --}}
...
<script>
    ...
    const getMaster = @json(route('saleswopo.master'));
    const getDetails = @json(route('saleswopo.details'));
    let table;
    let searchValue = "";

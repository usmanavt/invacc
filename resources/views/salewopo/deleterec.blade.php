<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div style="font-size: 3rem;font-weight: bold;color:brown;border:blue">
            Delete Record Information For Sale W/O P.O
        </div>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1">
                        ...
                        <div class="grid grid-cols-12 gap-2 py-2 items-center">
                            ...
                            <x-input-text title="P.O Seq.#" name="poseqno" id="poseqno" value="{{ $customerorder->poseqno }}"    required   /> --}}
                                ...
                                <x-input-text title="G.Pass No" name="gpno" id="gpno" value="{{ $saleinvoices->gpno }}"     disabled   />
                                <x-input-date title="Deilivery Date" id="deliverydt" name="deliverydt"  class="col-span-2" value="{{ $saleinvoices->saldate->format('Y-m-d') }}" disabled />
                                {{-- <x-input-text title="prvscustno" name="pcustno" id="pcustno" value="{{ $saleinvoices->customer_id }}"   /> --}}


                        </div>
                        <div class="grid grid-cols-12 gap-1 py-2 items-center">
                            <x-input-text title="DC No" name="dcno" id="dcno" value="{{ $saleinvoices->dcno }}"    class="col-span-2" disabled  />
                            <x-input-text title="Bill No" name="billno" id="billno"   value="{{ $saleinvoices->billno }}"  class="col-span-2" disabled  />
                                <label for="">
                                    Descripiton <span class="text-red-500 font-semibold"></span>
                                </label>
                                <textarea name="saldescription" id="saldescription" cols="100" rows="2" maxlength="150" disabled class="rounded"> {{ $saleinvoices->saldescription }}  </textarea>
                        ...
                        <fieldset class="border px-4 py-2 rounded">
                            ...
                            <div class="grid grid-cols-12 gap-2 py-2 items-center">
                                <x-input-numeric title="Discou(%)" name="discntper" id="discntper" value="{{ $saleinvoices->discntper }}" disabled    />
                                    {{-- <div class="basis-0 md:basis-1/5 self-center pt-4"> --}}
                                        <input class="checked:bg-blue-500 checked:border-blue-500 focus:outline-none" disabled type="checkbox" name="per" id="per" onclick="EnableDisableTextBox(this)" >
                                ...
                                <x-input-numeric title="Discount(Amount)" name="discntamt" id="discntamt" class="col-span-2" value="{{ $saleinvoices->discntamt }}" disabled    />
                            ...
                            <div class="grid grid-cols-12 gap-2 py-2 items-center">
                                <x-input-numeric title="Sale Tax(%)" name="saletaxper" class="col-span-2" value="{{ $saleinvoices->saletaxper }}"   onblur="tnetamount()" disabled  />
                                <x-input-numeric title="Sale Tax(Rs)" name="saletaxamt" class="col-span-2" value="{{ $saleinvoices->saletaxamt }}" disabled    />
                                <x-input-numeric title="Cartage" name="cartage" class="col-span-2" value="{{ $saleinvoices->cartage }}"  required  onblur="tnetamount()" disabled  />
                            </div>
                            <div class="grid grid-cols-12 gap-2 py-2 items-center">
...
function validateForm()
{
    ...
    if(customer_id.value < 0)
        ...
        if(futgp1.value == sale_invoice_id.value   )
        {
            showSnackbar("First of all Delete Data From Godown Gatepass of this Invoice");
            dcno.focus();
            return;
        }


        if(rcvvchr.value == sale_invoice_id.value   )
        {
        ...
        if(pmtvchr.value == sale_invoice_id.value   )
        {

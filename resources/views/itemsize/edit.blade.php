            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="px-12 py-2" >
                    <div class="flex flex-col justify-start items-center">
                        <form action="{{route('itemsize.update',$itemsize)}}" method="post" >
                            @csrf
                            @method('PUT')
                                <x-label for="sizename" :value="__('Item Size')" class="w-24"/>
                                <x-input id="sizename" class="bg-indigo-100" type="text" name="sizename" value="{{$itemsize->sizename}}" />
                                @if($errors->has('sizename'))<div class="text-red-500 text-xs">{{ $errors->first('sizename') }}</div>@endif

                                <x-label for="sizenname" :value="__('Knick Name')" class="w-24"/>
                                <x-input id="sizenname" class="bg-indigo-100" type="text" name="sizenname" value="{{$itemsize->sizenname}}" />

                ...
                <div class="px-6 py-2" >
                    ...
                    <div class="flex gap-8">
                        ...
                        <div class="flex flex-col justify-start items-center">
                            <form action="{{ route('hscodes.update',$hscode) }}" method="post" >
                            @csrf
                            @method('PUT')
                                <x-label for="hscode" value="Hscode Number"/>
                                <x-input id="hscode" class="bg-indigo-100" type="text" name="hscode" value="{{ $hscode->hscode }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('hscode'))<div class="text-red-500 text-xs">{{ $errors->first('hscode') }}</div>@endif
                                
                                <x-label for="cd" value="CD"/>
                                <x-input id="cd" class="bg-indigo-100" type="number" step="0.01" name="cd" value="{{ $hscode->cd }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('cd'))<div class="text-red-500 text-xs">{{ $errors->first('cd') }}</div>@endif
                                
                                <x-label for="st" value="ST"/>
                                <x-input id="st" class="bg-indigo-100" type="number" step="0.01" name="st" value="{{ $hscode->st }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('st'))<div class="text-red-500 text-xs">{{ $errors->first('st') }}</div>@endif
                                
                                <x-label for="rd" value="RD"/>
                                <x-input id="rd" class="bg-indigo-100" type="number" step="0.01" name="rd" value="{{ $hscode->rd }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('rd'))<div class="text-red-500 text-xs">{{ $errors->first('rd') }}</div>@endif
                                
                                <x-label for="acd" value="ACD"/>
                                <x-input id="acd" class="bg-indigo-100" type="number" step="0.01" name="acd" value="{{ $hscode->acd }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('acd'))<div class="text-red-500 text-xs">{{ $errors->first('acd') }}</div>@endif
                                
                                <x-label for="ast" value="AST"/>
                                <x-input id="ast" class="bg-indigo-100" type="number" step="0.01" name="ast" value="{{ $hscode->ast }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('ast'))<div class="text-red-500 text-xs">{{ $errors->first('ast') }}</div>@endif
                                
                                <x-label for="it" value="IT"/>
                                <x-input id="it" class="bg-indigo-100" type="number" step="0.01" name="it" value="{{ $hscode->it }}"  required minlength="1" title="numeric input required"/>
                                @if($errors->has('it'))<div class="text-red-500 text-xs">{{ $errors->first('it') }}</div>@endif
                                
                                <x-label for="wse" value="WSE"/>
                                <x-input id="wse" class="bg-indigo-100" type="number" step="0.01" name="wse" value="{{ $hscode->wse }}"  required minlength="1" title="numeric input required"/>

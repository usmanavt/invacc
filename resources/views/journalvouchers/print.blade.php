        ...
        <tbody>
            ...
            {{-- <tr>
                <td  style="text-align: right;">
                    <span style="font-size:1rem;font-weight: bold"> Date:  {{ $data[$i]->document_date }} </span>
                </td>
            </tr> --}}
...
<table class="voucher-headers">
    ...
    <table style="font-weight: bold">
        <tbody>
            <tr> <td> VOUCHER #: {{ $data[0]->jvno }}  </td>
                <td style="text-align: right" > DATE #:{{ $data[0]->document_date }}   </td>
            </tr>
    ...
    <table class="column-headers" cellspacing="0"  >
        <tbody>
            <tr>
                ...
                <td class="column-headers" cellspacing="0" width="65%">Description</td>
                <td class="column-headers" cellspacing="0" width="15%">Debit</td>
                <td class="column-headers" cellspacing="0" width="15%">Credit</td>
            </tr>
        </tbody>
    </table>
    ...
    <table class="data" cellspacing="0" >
        ...
        @for ($i = 0 ; $i < count($data) ; $i++)
            <tr>
                {{ $vardbt += $data[$i]->debit }}
                {{ $varcrdt += $data[$i]->credit }}

                <td class="" width="5%">{{ $i+1 }}</td>
                <td class="" width="65%">{{ $data[$i]->title }} <br>
                    <span style="font-size:0.8rem;font-weight: bold;color:brown"> {{ $data[$i]->description }}</span>
                </td>
                <td style="text-align: right" width="15%">{{ $data[$i]->debit }} </td>
                <td style="text-align: right" width="15%">{{ $data[$i]->credit }} </td>
            ...
            <tr>
                ...
                <td colspan="1"  style="text-align: right;border-bottom: 1px solid black;font-weight: bold">{{ number_format($vardbt,2) }} </td>
                <td colspan="1"  style="text-align: right;border-bottom: 1px solid black;font-weight: bold">{{ number_format($varcrdt,2) }} </td>
            </tr>
    ...
    <table >
        <tr style="padding:12px"> <td style="font-weight:bold"> Rupees.{{ $data[0]->amountword }} </td>   </tr>
    </table>
